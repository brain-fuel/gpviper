// Code generated by goplus from viper.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
//
//goplus:enum Resolution[T any]
type Resolution[T any] interface{ isResolution(T) }

//goplus:variant (Resolution[T]) ResolvedValue(Value T, Source Source)
type ResolvedValue[T any] struct {
	Value  T
	Source Source
}

func (ResolvedValue[T]) isResolution(T) {}

//goplus:variant (Resolution[T]) MissingResolution
type MissingResolution[T any] struct{}

func (MissingResolution[T]) isResolution(T) {}

// ResolutionCases selects one handler per Resolution variant for Fold.
type ResolutionCases[T any, R any] struct {
	ResolvedValue     func(Value T, Source Source) R
	MissingResolution func() R
}

// Fold reduces Resolution[T] by one-level case analysis.
func Fold[T any, R any](v Resolution[T], cs ResolutionCases[T, R]) R {
	switch m := any(v).(type) {
	case ResolvedValue[T]:
		return cs.ResolvedValue(m.Value, m.Source)
	case MissingResolution[T]:
		return cs.MissingResolution()
	default:
...
func (s Snapshot) Resolve(key string) Resolution[any] {
	if value, ok := s.Get(key); ok {
		return ResolvedValue[any]{Value: value.Value, Source: value.Source}
	}
	return MissingResolution[any]{}
}
